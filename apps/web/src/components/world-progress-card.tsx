...
// World colors for visual distinction
...
export function WorldProgressCard() {
  const { data: worlds, isLoading: worldsLoading } = useQuery(
    trpc.worlds.getAll.queryOptions()
  )
  ...
  return (
    <div className="flex flex-col gap-6">
      {worlds.map((world, index) => {
        ...
        // Calculate progress based on user XP
        let progress = 0
          ...
          >
            ...
            <div className="absolute right-4 top-1/2 -translate-y-1/2 hidden h-48 w-48 lg:flex items-center justify-center z-20">
              <div className={isLocked ? "grayscale opacity-50" : ""}>
                <img src={planetImage} alt={world.name} className="w-full h-full object-contain" loading="lazy" />
              </div>
            </div>
          </div>
        )
